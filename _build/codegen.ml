         ...
         and t2 = L.type_of e2' in 
	       ( match op with
	         A.Add     -> if t1 = float_t || t2 = float_t  then L.build_fadd 
                       ...
                       (* else if t1 = L.pointer_type i8_t || t2 = L.pointer_type i8_t  then *)
	        | A.Sub     -> if t1 = float_t || t2 = float_t then L.build_fsub  
                         else L.build_sub 
          | A.Mult    -> if t1 = float_t || t2 = float_t  then L.build_fmul 
                        else L.build_mul
          | A.Div     -> if t1 = float_t || t2 = float_t then L.build_fdiv 
                        else L.build_sdiv
	        | A.And     -> L.build_and 
	        | A.Or      -> L.build_or 
